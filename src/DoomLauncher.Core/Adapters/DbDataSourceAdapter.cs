    ...
    public class DbDataSourceAdapter : IDataSourceAdapter
    {
        ...
        private string InsertStatement(string tableName, object obj, string[] exclude, out List<DbParameter> parameters)
        {
            ...
            foreach(PropertyInfo pi in properties)
            {
                ...
                parameters.Add(DataAccess.DbAdapter.CreateParameter(pi.Name, ConvertInsertValue(pi.GetValue(obj))));
            }
        ...
        private static object ConvertInsertValue(object value)
        {
            if (value == null)
                return DBNull.Value;
            if (value is IWadInfo info)
                return (object)info.GameName ?? DBNull.Value;
            if (value is LauncherPath path)
                return path.GetPossiblyRelativePath() ?? (object)DBNull.Value;
            if (value is Enum)
