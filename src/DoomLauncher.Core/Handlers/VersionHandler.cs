    ...
    public class VersionHandler
    {
        ...
        public void Pre_0_9_2()
        {
            ...
            if (dt.Rows.Count == 0)
            {
                ...
                "GameFiles", "GameFiles/Screenshots", "GameFiles/Temp",
                "GameFiles", "GameFiles/Demos", Path.Combine("GameFiles", "SaveGames"));

                DataAccess.ExecuteNonQuery(query);

                DirectoryInfo di = new DirectoryInfo(Path.Combine(LauncherPath.GetDataDirectory(), "GameFiles", "SaveGames"));
                if (!di.Exists)
                    di.Create();
            }
