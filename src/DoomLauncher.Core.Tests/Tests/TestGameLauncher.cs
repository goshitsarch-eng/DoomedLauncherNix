...
namespace UnitTest.Tests
{
    ...
    public class TestGameLauncher
    {
        ...
        public void TestAdditionalFiles()
        {
            ...
            string temp = Path.Combine(Directory.GetCurrentDirectory(), "Temp");
            string check = string.Format(
                " -file \"{0}\" \"{1}\" \"{2}\" \"{3}\" \"{4}\" \"{5}\" \"{6}\" \"{7}\"  -deh \"{8}\" \"{9}\" \"{10}\" \"{11}\" ",
                Path.Combine(temp, "test2.wad"), Path.Combine(temp, "test2.pk3"),
                Path.Combine(temp, "test3.wad"), Path.Combine(temp, "test3.pk3"),
                Path.Combine(temp, "test4.wad"), Path.Combine(temp, "test4.pk3"),
                Path.Combine(temp, "test1.wad"), Path.Combine(temp, "test1.pk3"),
                Path.Combine(temp, "test2.deh"), Path.Combine(temp, "test3.deh"),
                Path.Combine(temp, "test4.deh"), Path.Combine(temp, "test1.deh"));
            Assert.AreEqual(check.Trim(), launch.Trim());
        ...
        {
            string filename = "GameFiles/testpathed.zip";
        ...
        private static void CreateTestFiles()
        {
            for (int i = 1; i < 5; i++)
            {
                ...
                string filename = string.Format("GameFiles/test{0}.zip", i);
                if (File.Exists(filename))
                    File.Delete(filename);
